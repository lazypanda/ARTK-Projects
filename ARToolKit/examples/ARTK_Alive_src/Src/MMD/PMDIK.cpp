	...
	for( unsigned short it = 0 ; it < m_unCount ; it++ )
	{
		for( unsigned char cbLinkIdx = 0 ; cbLinkIdx < m_cbNumLink ; cbLinkIdx++ )
		{
			...
			if( 0.00000001f < fabsf( fRotAngle ) )
			{
				...
				QuaternionMultiply( &m_ppBoneList[cbLinkIdx]->m_vec4Rotate, &m_ppBoneList[cbLinkIdx]->m_vec4Rotate, &vec4RotQuat );
				QuaternionNormalize( &m_ppBoneList[cbLinkIdx]->m_vec4Rotate, &m_ppBoneList[cbLinkIdx]->m_vec4Rotate );
