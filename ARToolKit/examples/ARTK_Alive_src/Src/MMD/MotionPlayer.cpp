	...
	else
	{
		...
		while( pMotionDataList )
		{
			if( *ppBone )
			{
				...
				QuaternionSlerp( &((*ppBone)->m_vec4Rotate), &((*ppBone)->m_vec4Rotate), &vec4Rotate, fInterpolateRate );
			}
...
void cMotionPlayer::getMotionPosRot( const MotionDataList *pMotionData, float fFrame, Vector3 *pvec3Pos, Vector4 *pvec4Rot )
{
	...
	if( lKey0 != lKey1 )
	{
		...
		Vector3Lerp( pvec3Pos, &(pMotionData->pKeyFrames[lKey0].vec3Position), &(pMotionData->pKeyFrames[lKey1].vec3Position), fLerpValue );
		QuaternionSlerp( pvec4Rot, &(pMotionData->pKeyFrames[lKey0].vec4Rotate), &(pMotionData->pKeyFrames[lKey1].vec4Rotate), fLerpValue );
